...
class GPT2Layer(nn.Module):
    ...
    def add(self, hidden_in, hidden_out, dense_layer, dropout):
        hidden_out = dense_layer(hidden_out)    # FNN
        hidden_out = dropout(hidden_out)    # Dropout
        return hidden_in + hidden_out   # Residual Connection
    ...
    def forward(self, hidden_states, attention_mask=None):
        ...
        normed = self.attention_layer_norm(hidden_states)   # Pre-LN
        ...
        normed = self.out_layer_norm(hidden_states) # Pre-LN
        ff_out = self.interm_dense(normed)   # c_fc
        ff_out = self.interm_af(ff_out)      # GELU
        hidden_states = self.add(
            hidden_states, 
            ff_out, 
